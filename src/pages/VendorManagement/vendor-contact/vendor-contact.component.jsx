...
import AddVendorContactComponent from './components/add-vendorcontact.component';
...
const VendorContactList = () => {
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      await VendorContactAPI.deleteVendorContact(recordId.current);
      addToast({ message: Message.DELETE_SUCCESS, type: 'success' });
      await getListVendorContact();
  ...
  const getListVendorContact = useCallback(async () => {
    try {
      setLoading(true);
      const response = await VendorContactAPI.getVendorContactList();
  ...
  return (
    <React.Fragment>
      ...
      <div className='page-content'>
        <Container fluid>
          <BreadCrumb title='Danh sách liên hệ' pageTitle='Quản lý số' />
          <Row>
            <Col lg={12}>
              <Card>
                <CardHeader>
