...
const RoutingList = () => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (openModal.updateRecord) {
          ...
          await RoutingAPI.createNewRouting(values);
        }
        addToast({
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      await RoutingAPI.deleteRouting(recordId.current);
  ...
  const getListRouting = useCallback(async () => {
    try {
      setLoading(true);
      const response = await RoutingAPI.getRoutingList();
  ...
  return (
    <React.Fragment>
      ...
      <div className='page-content'>
        <Container fluid>
          <BreadCrumb title='Routing' pageTitle='Partner Management' />
          <Row>
            <Col lg={12}>
              <Card>
                <CardHeader>
      ...
      >
        <ModalHeader>
          {openModal.isUpdate ? 'Update Routing' : 'Create New Routing'}
          <Button
            type='button'
            onClick={close}
            className='btn-close'
