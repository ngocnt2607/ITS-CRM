...
function AddSmsBrand({
  ...
  const getListData = useCallback(async () => {
    const listAPI = [SmsBrandAPI.getSmsBrandList];
    try {
      setLoading(true);
  ...
  const handleClickConfirm = async () => {
    ...
    try {
      ...
      if (activeTab === '1') {
        if (updateRecord) {
          await SmsBrandAPI.updateSmsBrand({
            ...values,
            id: updateRecord.id,
          });
        } else {
          await SmsBrandAPI.createNewSmsBrand({
            ...values,
          });
        }
        addToast({
        ...
        if (updateRecord1) {
          ...
          await SmsBrandAPI.createSmsBrandTopUp({
            ...values,
            customer_code: values.customer_code?.value,
          });
        }
...
export default React.memo(AddSmsBrand);