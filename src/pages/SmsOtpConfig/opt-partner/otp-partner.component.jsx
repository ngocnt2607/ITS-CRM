...
import AddOtpComponent from './components/add-otppartner.component';
...
const OtpList = () => {
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      await OtpAPI.deleteVBN(recordId.current);
  ...
  const convertData = (data) => {
    return (
      data?.map((item) => {
        ...
        return {
          ...item,
          currentbalance: nf.format(item?.['currentbalance']),
          limitbalance: nf.format(item?.['limitbalance']),
          todayconsumption: nf.format(item?.['todayconsumption']),
        };
      }) || []
    );
  };

  const getListOtp = useCallback(async () => {
    try {
      setLoading(true);
      const response = await Promise.all([OtpAPI.getOTPList()]);
  ...
  const [columnConfig, setColumnConfig] = useState([
    ...
    },
    {
      ...
      headerName: 'Currentbalance',
      width: 150,
    },
    {
      field: 'limitbalance',
      headerName: 'Limitbalance',
      width: 150,
    },
    {
      field: 'todayconsumption',
      headerName: 'Todayconsumption',
      width: 180,
    },
    {
      field: 'link',
  ...
  document.title = 'Quản lý danh sách OTP Partner';
  ...
  return (
    <React.Fragment>
      ...
      <div className='page-content'>
        <Container fluid>
          <BreadCrumb
            title='Danh sách OTP Partner'
            pageTitle='Quản lý OTP Partner'
          />
          <Row>
            <Col lg={12}>
              <Card>
